import React from "react";

const ClientLogos = ({ title, clients }) => {
  // clients should be an array of objects: [{ name: 'Client Name', logoUrl: 'path/to/logo.svg' }]
  return (
    <div className="bg-gray-50 py-12 sm:py-16">
      <div className="mx-auto max-w-screen-xl px-6 lg:px-8">
        <h2 className="text-center font-montserrat text-3xl font-bold leading-8 text-[#0D2C54]">
          {title}
        </h2>
        <div className="mx-auto mt-10 grid max-w-lg grid-cols-2 items-center gap-x-8 gap-y-12 sm:max-w-xl sm:grid-cols-3 lg:mx-0 lg:max-w-none lg:grid-cols-5">
          {clients.map((client) => (
            <img
              key={client.name}
              className="col-span-1 max-h-12 w-full object-contain grayscale transition hover:grayscale-0"
              src={client.logoUrl}
              alt={client.name}
              width={158}
              height={48}
            />
          ))}
        </div>
      </div>
    </div>
  );
};

export default ClientLogos;
